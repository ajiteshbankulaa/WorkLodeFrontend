...
type RiskSort = "flagged" | "course" | "hours" | "difficulty";
...
export function Insights() {
  ...
  const [riskSort, setRiskSort] = useState<RiskSort>("flagged");
  ...
  const sortedRiskRows = useMemo(() => [...riskRows].sort((a, b) => {
    if (riskSort === "course") return a.course_code.localeCompare(b.course_code);
    if (riskSort === "hours") return (b.avg_hours ?? -1) - (a.avg_hours ?? -1);
    if (riskSort === "difficulty") return (b.avg_difficulty ?? -1) - (a.avg_difficulty ?? -1);
    return b.count - a.count;
  }), [riskRows, riskSort]);
  const adminChart = useMemo(() => {
    if (sortedRiskRows.length > 0) {
      return sortedRiskRows.map((row) => ({
  ...
  }, [sortedRiskRows, departmentCourses, adminChartMetric]);
  ...
  const adminActionQueue = useMemo(() => {
    ...
    if (sortedRiskRows[0]) actions.push({ title: "Top flagged course", body: `${sortedRiskRows[0].course_code} is the strongest current workload signal in this view.` });
  ...
  }, [sortedRiskRows, departmentCourses]);
  ...
  if (!isAuthenticated) return <div className="min-h-screen px-4 py-10"><div className="container mx-auto max-w-2xl"><div className="rounded-[28px] border border-border bg-white p-8 text-center shadow-sm"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-secondary/10 text-secondary"><Brain size={24} /></div><h1 className="mt-5 text-3xl font-black tracking-tight text-text">Insights requires sign-in</h1><p className="mt-2 text-sm text-text-secondary">Sign in with Google to open the planning analysis and admin insight workspace.</p><GoogleSignInButton className="mt-6 inline-flex bg-secondary px-4 py-3 text-white hover:bg-secondary-hover" label="Sign in with Google" />{authError ? <div className="mt-6 text-sm text-red-600">{authError}</div> : null}</div></div></div>;
